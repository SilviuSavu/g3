...
| Variable | Purpose |
|----------|---------|
| `G3_WORKSPACE_PATH` | Override the workspace directory |
| `G3_TODO_PATH` | Custom path for todo file in planning mode |
| `G3_RETRO_MODE` | When set, suppresses emoji and colored output in OAuth flows (for terminal compatibility) |
| `OPENROUTER_API_KEY` | Fallback API key for embeddings when not specified in config |
